...
def create_orders(order_rules: list, pages: list) -> list:
    ...
    for i in pages:
        ...
        check_val = all([(x in order_rules) for x in page_orderings])
        print(f"allowed: {check_val}")
        if check_val:
            middle_val = i[len(i) // 2]
            middle_num_of_correct_orders.append(middle_val)
    ret = sum(middle_num_of_correct_orders)
    return ret


example_num = create_orders(order_rules=allowed_orders, pages=example_bottom)
print(example_num)
...
pt1_num = create_orders(order_rules=pt1_allowed_orders, pages=pt1_bottom)
print(pt1_num)
